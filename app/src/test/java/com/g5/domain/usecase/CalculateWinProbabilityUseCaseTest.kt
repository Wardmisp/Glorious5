...
/**
 * getPercentile/calculatePlayerScore/calculateTeamScore sont privées : on les vérifie en boîte
 * noire via execute(), en choisissant des jeux de données dont le pourcentile de chaque
 * statistique est calculable à la main plutôt qu'en se contentant de bornes.
 */
class CalculateWinProbabilityUseCaseTest {
    ...
    fun execute_playerStrictlyAheadOnEveryStat_scoresExactly75thPercentileOnEveryComponent() {
        // Distribution à 2 joueurs : quand toutes les stats de X dominent strictement celles de Y,
        // chaque pourcentile de X vaut (countBelow=1 + 0.5*countEqual=1) / 2 * 100 = 75, et celui
        // de Y vaut (0 + 0.5*1) / 2 * 100 = 25 — pour CHAQUE statistique, donc aussi pour EFF,
        // IMPACT et le score total (dont les poids somment à 1.0).
        val strong = player(
            id = 1, position = "Meneur",
    ...
    fun execute_teamScoreIsTotalScoreWeightedByPositionalImportance() {
        ...
        // totalScore = 75 pour "meneur" (poids 0.22) et 25 pour "pivot" (poids 0.18) — meneur/pivot
        // dominent (ou sont dominés) sur CHAQUE statistique, cf. le test précédent.
    ...
    fun execute_unknownPosition_fallsBackToDefaultWeightOfTwentyPercent() {
        // Distribution à un seul joueur, lui-même : chaque pourcentile vaut (0 + 0.5*1)/1*100 = 50,
        // donc totalScore = 50.0 exactement (les poids des composantes somment à 1.0).
        val soloPlayer = player(id = 1, position = "Poste Inconnu", pts = 15.0, reb = 6.0, ast = 4.0, stl = 1.0, blk = 1.0)
    ...
    fun execute_zeroGamesPlayed_treatsWinSharesPerGameAsZeroInsteadOfCrashing() {
        // games = 0 doit court-circuiter la division par zéro (wsPerGame = 0.0) plutôt que planter,
        // et rester traitable comme n'importe quelle autre statistique par getPercentile.
        val neverPlayed = player(id = 1, position = "Ailier", pts = 5.0, reb = 2.0, ast = 1.0, stl = 0.1, blk = 0.1, winShares = 9.0, games = 0)

        val (analyticsA, _) = useCase.execute(teamA = listOf(neverPlayed), teamB = emptyList(), allSeasons = listOf(neverPlayed))

        // Distribution à un seul joueur -> 50e percentile partout, y compris pour WS/Game.
    ...
    fun execute_winProbabilitiesAreAlwaysComplementary() {
        val strong = player(id = 1, position = "Meneur", pts = 30.0, reb = 12.0, ast = 8.0, stl = 3.0, blk = 2.0)
        val weak = player(id = 2, position = "Pivot", pts = 10.0, reb = 4.0, ast = 2.0, stl = 0.5, blk = 0.2)
    ...
    fun execute_identicalTeams_giveEvenOdds() {
        val player = player(id = 1, position = "Ailier", pts = 20.0, reb = 5.0, ast = 5.0, stl = 1.0, blk = 1.0)
